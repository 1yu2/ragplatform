...
async def tavily_search(
    ...
    # Initialize summarization model with retry logic (no structured output for compatibility)
...
async def summarize_webpage(model: BaseChatModel, webpage_content: str) -> str:
    ...
    try:
        ...
        # Extract content from response (handle both structured and non-structured output)
        ...
        try:
            # Look for JSON object in the response (handle markdown code blocks)
            json_match = re.search(r'\{[^{}]*\}', content, re.DOTALL)
            if json_match:
                data = json.loads(json_match.group())
                summary_text = data.get('summary', content)
                key_excerpts = data.get('key_excerpts', '')
            else:
                # No JSON found, use the content as-is
                summary_text = content
                key_excerpts = ''
        except (json.JSONDecodeError, AttributeError):
            # Failed to parse JSON, use the content directly
            summary_text = content
            key_excerpts = ''
